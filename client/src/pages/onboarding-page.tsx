...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      if (!user) {
        setDebugMessage('No user in store, checking session...');
        const { data: { session } } = await supabase.auth.getSession();
        if (session?.user) {
          setDebugMessage('Session found! Reinitializing...');
          await initialize();
        } else {
          setDebugMessage('No session found');
        }
      } else {
        setDebugMessage('User loaded: ' + user.email);
      }
    };
    checkUser();
  }, [user, initialize]);
  ...
  const uploadAvatar = async (): Promise<string | null> => {
    ...
    try {
      setDebugMessage('Uploading to storage...');
  ...
  const handleComplete = async () => {
    try {
      ...
      if (!user) {
        setDebugMessage('No user found! Checking session...');
        const { data: { session } } = await supabase.auth.getSession();
        if (!session?.user) {
          toast({
            title: 'Session Error',
  ...
  // Check if user exists - with back button
  if (!user) {
    return (
        ...
        <Card className="w-full max-w-md">
          <CardContent className="pt-6 space-y-4">
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {debugMessage || 'No user session found. Please log in again.'}
              </AlertDescription>
            </Alert>
            <Button 
